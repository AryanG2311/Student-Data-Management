    ...
    <div className="flex min-h-screen bg-slate-50 font-sans text-slate-800">
      ...
      <aside className="fixed inset-y-0 left-0 z-20 flex w-64 flex-col border-r border-slate-200 bg-white shadow-sm">
        ...
        <div className="flex h-16 items-center gap-2.5 border-b border-slate-200 px-5">
          <img src="/dtu-logo (1).png" alt="DTU Logo" className="h-8 w-auto object-contain shrink-0" />
      ...
      {/* Main Content Area */}
      <div className="flex flex-1 flex-col pl-64 min-h-screen bg-slate-50">
        {/* Top Header */}
        ...
        <main className="flex-1 p-6 space-y-6">
          {/* DTU Campus Hero Banner */}
          <div 
            className="w-full h-55 rounded-md overflow-hidden relative flex flex-col justify-end p-5 shadow-sm border border-slate-200"
            style={{
              backgroundImage: "linear-gradient(to top, rgba(15, 23, 42, 0.65), rgba(15, 23, 42, 0.15)), url('/dtu bg2.webp')",
              backgroundSize: 'cover',
              backgroundPosition: 'center 65%',
              backgroundRepeat: 'no-repeat'
            }}
          >
            <div className="z-10 space-y-1.5 p-1">
              <h2 className="text-2xl font-black tracking-wider text-white uppercase drop-shadow-sm leading-tight">
                Delhi Technological University
              </h2>
              <p className="text-sm text-slate-200 font-medium tracking-wide drop-shadow-sm opacity-95">
                Student Data Pipeline & Recruitment Control Center
              </p>
            </div>
          </div>
